<template>
    <div class="relative">
        ...
        <form @submit.prevent="editContent" class="flex flex-wrap justify-around pb-6">
            <div class="w-full ml-4">
                <router-link :to="{ name: 'AdminContents' }" style="cursor: pointer;margin: auto 0px;"
                    class="material-icons-sharp">arrow_back</router-link>
            </div>
            <div class="w-full">
            ...
            <select v-model="content.category_id"
                class="w-[48%] appearance-none px-2 my-6 bg-transparent border text-[14px] rounded-md outline-none border-primary">
                <option disabled selected>Choose a category</option>
                <option v-for="cat in categories" :key="cat.id" :value="cat.id">{{ cat.name }}</option>
            </select>

            <select v-model="content.status"
                class="w-[48%] appearance-none p-2 my-6 bg-transparent border text-[14px] rounded-md outline-none border-primary">
            ...
            <div class="w-[98%] mb-32 mt-2 mx-auto">
                ...
                <quill-editor class="w-full shadow-sm shadow-primary" v-model:content="content.description" theme="snow"
                    toolbar="full" contentType="html"></quill-editor>
            </div>

            <div class="w-[98%] mx-auto mb-32">
                <label for="description">Paragraph</label>
                <quill-editor class="w-full shadow-sm shadow-primary" v-model:content="content.paragraph" theme="snow"
                    toolbar="full" contentType="html"></quill-editor>
            </div>
...
export default {
    ...
    methods: {
        ...
        editContent() {
            ApiService.patch(`admin/contents/${this.$route.params.id}`, this.content).then(() => {
    ...
    mounted() {
        this.getCategory()
        ApiService.get(`admin/contents/${this.$route.params.id}`).then((res) => {
